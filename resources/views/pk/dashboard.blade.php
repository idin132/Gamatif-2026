...
<head>
    ...
    <style>
        ...
        .input-glass:focus { border-color: rgba(201,168,76,0.5); }
        .input-glass::placeholder { color: rgba(161,161,170,0.4); }
...
<body class="page-bg text-zinc-100 font-inter pb-24 select-none min-h-screen"
    ...
    <div class="topbar sticky top-0 z-40 px-4 py-2 flex items-center justify-between">
        ...
        <div class="relative" x-data="{ open: false }" @click.outside="open = false">
            <button @click="open = !open"
                    class="flex items-center gap-1.5 px-2 py-1 rounded-full transition"
                    style="border: 1px solid rgba(201,168,76,0.35); background: rgba(201,168,76,0.08);">
                {{-- Avatar inisial --}}
                <span class="w-6 h-6 rounded-full flex items-center justify-center text-[0.55rem] font-black text-black shrink-0"
            ...
            <div x-show="open"
                 ...
                 class="absolute right-0 mt-1.5 w-36 rounded-lg overflow-hidden z-50"
                ...
                <div class="px-3 py-2 border-b" style="border-color: rgba(201,168,76,0.12);">
                    ...
                    <p class="text-[0.55rem] tracking-widest uppercase mt-0.5" style="color: rgba(201,168,76,0.5);">House Leader</p>
                </div>
                {{-- Logout --}}
                <div class="py-1">
    ...
    <main class="pk-main px-4 pt-4 pb-2 space-y-3">
        ...
        <div x-show="activeTab === 'bawaan'" class="space-y-4">
            ...
            <p class="text-[10px] text-zinc-500 -mt-1 px-0.5">Tap B1–B5 untuk toggle status bawa. <span class="text-emerald-500/70">Hijau = Bawa.</span></p>
            ...
            <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-5 gap-2">
                            @for($i = 0; $i < 5; $i++)
                                ...
                                <button type="button"
                                        ...
                                        data-day_1="{{ $row->$colDay1 }}"
                                        data-day_2="{{ $row->$colDay2 }}"
                ...
                @empty
                    <div class="card-glass rounded-lg py-6 px-4 text-center" style="border-color:rgba(201,168,76,0.08);">
                        <svg class="w-7 h-7 mx-auto mb-2 text-gold-500/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div x-show="activeTab === 'absensi'" class="space-y-4">
            ...
            <div class="grid grid-cols-3 gap-1.5 p-1 card-glass rounded-lg">
                <button @click="activeDay = 'day_1'"
                        :class="activeDay === 'day_1' ? 'day-tab-active' : 'day-tab-inactive'"
                        class="py-1.5 text-[0.65rem] rounded-md transition font-cinzel tracking-widest text-center">Day 1</button>
                <button @click="activeDay = 'day_2'"
                        :class="activeDay === 'day_2' ? 'day-tab-active' : 'day-tab-inactive'"
                        class="py-1.5 text-[0.65rem] rounded-md transition font-cinzel tracking-widest text-center">Day 2</button>
                <button @click="activeDay = 'day_3'"
                        :class="activeDay === 'day_3' ? 'day-tab-active' : 'day-tab-inactive'"
                        class="py-1.5 text-[0.65rem] rounded-md transition font-cinzel tracking-widest text-center">Day 3</button>
            </div>
            ...
            <div class="space-y-3">
                @forelse($dataMahasiswas as $row)
                    <div class="card-glass p-3.5 rounded-lg flex items-center justify-between">
                        <div>
                            <h4 class="font-cinzel font-bold text-xs text-white">{{ $row->nama }}</h4>
                            <span class="text-[10px] font-mono text-gold-500/70">{{ $row->nim }}</span>
                        </div>
                        <select onchange="updateKehadiran({{ $row->id }}, this.value)"
                                class="input-glass w-auto min-w-[90px]">
        ...
        <div x-show="activeTab === 'izin'" class="space-y-4">
            <div class="card-glass p-5 rounded-lg">
                ...
                <form action="{{ route('pk.store_izin') }}" method="POST"
                    ...
                    <div>
                        ...
                        <textarea name="catatan" rows="2" required
                                  placeholder="Alasan sakit/izin..."
                                  class="input-glass resize-none"></textarea>
                    </div>

                    <button type="submit"
                            class="w-full py-3 rounded-sm text-xs font-cinzel tracking-widest uppercase
        ...
        <div x-show="activeTab === 'maba'" class="space-y-3">
            @forelse($mabas as $maba)
                <div class="card-glass p-3.5 rounded-lg flex items-center justify-between">
                    ...
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $maba->nomor_whatsapp) }}"
                       target="_blank"
                       class="px-3 py-1.5 rounded-sm text-xs font-cinzel tracking-wider uppercase
    ...
    <nav class="bottom-nav fixed bottom-0 left-0 right-0 flex justify-around py-2 z-50">
        ...
        <button @click="activeTab = 'bawaan'"
                :class="activeTab === 'bawaan' ? 'text-gold-400' : 'text-zinc-600'"
                class="relative flex flex-col items-center text-[10px] gap-1 font-cinzel tracking-wider uppercase transition pt-1">
            <span x-show="activeTab === 'bawaan'" class="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-b bg-gold-500" style="box-shadow:0 0 6px rgba(201,168,76,0.6);"></span>
        ...
        <button @click="activeTab = 'absensi'"
                :class="activeTab === 'absensi' ? 'text-gold-400' : 'text-zinc-600'"
                class="relative flex flex-col items-center text-[10px] gap-1 font-cinzel tracking-wider uppercase transition pt-1">
            <span x-show="activeTab === 'absensi'" class="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-b bg-gold-500" style="box-shadow:0 0 6px rgba(201,168,76,0.6);"></span>
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
        ...
        <button @click="activeTab = 'izin'"
                :class="activeTab === 'izin' ? 'text-gold-400' : 'text-zinc-600'"
                class="relative flex flex-col items-center text-[10px] gap-1 font-cinzel tracking-wider uppercase transition pt-1">
            <span x-show="activeTab === 'izin'" class="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-b bg-gold-500" style="box-shadow:0 0 6px rgba(201,168,76,0.6);"></span>
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
        ...
        <button @click="activeTab = 'maba'"
                :class="activeTab === 'maba' ? 'text-gold-400' : 'text-zinc-600'"
                class="relative flex flex-col items-center text-[10px] gap-1 font-cinzel tracking-wider uppercase transition pt-1">
            <span x-show="activeTab === 'maba'" class="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-b bg-gold-500" style="box-shadow:0 0 6px rgba(201,168,76,0.6);"></span>
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
    ...
    <script>
        ...
        function updateKehadiran(id, status) {
            ...
            fetch("{{ route('pk.update_kehadiran') }}", {
                method: "POST",
                headers: { "Content-Type": "application/json", "X-CSRF-TOKEN": "{{ csrf_token() }}" },
                body: JSON.stringify({ id: id, day: currentDay, status: status })
            })
            .then(res => res.json())
            .catch(err => console.error(err));
        }
