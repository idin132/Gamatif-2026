...
<body class="page-bg text-zinc-100">
    ...
    <header id="navbar" class="fixed top-0 inset-x-0 z-50 transition-all duration-500"
        style="background: rgba(10,8,6,0.88); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(201,168,76,0.15);">
        <div class="max-w-screen-xl mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
            ...
            <a href="{{ route('landing') }}" class="shrink-0">
                <img src="{{ asset('images/logo-gamatif.png') }}" alt="GAMATIF 2026"
            ...
            @auth('peserta')
                <div class="relative hidden md:block" x-data="{ open: false }" @click.outside="open = false">
                    <button @click="open = !open" class="flex items-center gap-2 px-3 py-1.5 rounded-full transition"
                        ...
                        <svg class="w-3 h-3 text-gold-400 transition-transform" :class="{ 'rotate-180': open }" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div x-show="open" x-transition class="absolute right-0 mt-2 w-52 rounded-lg overflow-hidden z-50"
                        ...
                        <div class="px-4 py-3 border-b border-gold-500/15">
                            <p class="font-semibold text-white text-sm">{{ Auth::guard('peserta')->user()->nama_lengkap }}
                            </p>
                            <p class="text-xs text-zinc-500 mt-0.5">NIM: {{ Auth::guard('peserta')->user()->nim }}</p>
                        </div>
                        <div class="py-1">
                            <a href="{{ route('peserta.dashboard') }}"
                                class="flex items-center px-4 py-2.5 text-sm text-zinc-300 hover:text-white hover:bg-white/5 transition">Dashboard</a>
                            <a href="{{ route('peserta.profil') }}"
                                class="flex items-center px-4 py-2.5 text-sm text-zinc-300 hover:text-white hover:bg-white/5 transition">Profil
                                Saya</a>
                        </div>
                        <div class="border-t border-gold-500/15 py-1">
                            <form method="POST" action="{{ route('peserta.logout') }}">
                                @csrf
                                <button type="submit"
                                    class="w-full text-left px-4 py-2.5 text-sm text-rose-400 hover:text-rose-300 hover:bg-white/5 transition">Logout</button>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <button id="mobileMenuBtn" class="md:hidden text-gold-300 p-2" aria-label="Menu">
                <svg id="iconOpen" class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.5"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
                <svg id="iconClose" class="w-6 h-6 hidden" fill="none" stroke="currentColor" stroke-width="1.5"
                    viewBox="0 0 24 24">
        ...
        <div id="mobileMenu"
            ...
            <div class="pt-4 flex flex-col gap-3">
                @auth('peserta')
                    <a href="{{ route('peserta.dashboard') }}"
                        class="btn-glass text-center px-5 py-3 rounded-sm">Dashboard</a>
                    <form method="POST" action="{{ route('peserta.logout') }}">
                        @csrf
                        <button type="submit"
                            class="btn-glass-outline w-full text-center px-5 py-3 rounded-sm">Logout</button>
                    </form>
                @else
                    <a href="{{ route('peserta.login') }}"
                        class="btn-glass-outline text-center px-5 py-3 rounded-sm">Masuk</a>
                    <a href="{{ route('peserta.register') }}"
                        class="btn-glass text-center px-5 py-3 rounded-sm">Registrasi</a>
                @endauth
            </div>
        </div>
    </header>
    ...
    <main class="pt-32 pb-20 px-6 lg:px-10">
        <div class="max-w-screen-xl mx-auto">
            ...
            <div class="text-center mb-12 animate-fade-up">
                ...
                <p class="text-zinc-400 text-sm max-w-sm mx-auto">Klik salah satu kartu untuk post di story Instagram!
                </p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                @forelse($menfesses as $i => $mf)
                    ...
                    <div class="menfess-card group relative rounded-2xl p-6 cursor-pointer reveal
                                        transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02]" style="background: {{ $skin['bg'] }};
                                        border: 1px solid {{ $skin['border'] }};
                                        box-shadow: 0 4px 20px rgba(0,0,0,0.5);
                                        transition-delay: {{ ($i % 6) * 60 }}ms" data-from="{{ $mf->from }}"
                        data-to="{{ $mf->to }}" data-message="{{ $mf->message }}" data-accent="{{ $skin['accent'] }}">
                        ...
                        <div class="absolute inset-0 rounded-2xl flex flex-col items-center justify-center gap-3
                            ...
                            <button type="button" onclick="shareCardToIG(this.closest('.menfess-card'))"
    ...
    <div id="storyExportContainer" class="fixed top-0 left-0 overflow-hidden pointer-events-none"
        ...
        <div id="storyExportBg" class="w-full h-full flex items-center justify-center p-6"
            ...
            <div id="storyExportCard" class="w-full flex flex-col rounded-2xl p-8 text-center" style="background: linear-gradient(135deg, rgba(255,255,255,0.08), rgba(255,255,255,0.02));
                        border: 1px solid rgba(201,168,76,0.4);
                        box-shadow: 0 8px 32px rgba(0,0,0,0.6);">
                <div class="flex-grow">
                    <p style="color:rgba(161,161,170,0.7);font-size:14px;margin-bottom:6px;">Pesan untuk</p>
                    <h2 id="export-to"
                        style="font-size:28px;font-weight:700;color:#E8D49E;margin:6px 0 16px;word-break:break-word;font-family:serif;">
                    </h2>
                    <p id="export-message"
                        style="font-size:18px;color:#e4e4e7;line-height:1.7;margin:16px 0 24px;word-break:break-word;font-style:italic;font-family:serif;">
                    </p>
                    <p id="export-from"
                        style="font-size:16px;color:rgba(161,161,170,0.7);font-weight:600;word-break:break-word;"></p>
                </div>
                <div style="margin-top:24px;padding-top:16px;border-top:1px solid rgba(201,168,76,0.25);">
                    <p style="font-family:serif;font-size:24px;color:#C9A84C;font-weight:700;letter-spacing:4px;">
                        GAMATIF 2026</p>
                </div>
            </div>
        </div>
    ...
    <div id="modalMenfess" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4"
        ...
        <div class="w-full max-w-md card-glass rounded-2xl p-8 relative">
            ...
            <form action="{{ route('kirim_menfess') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label
                        class="font-cinzel text-[0.6rem] tracking-widest uppercase text-gold-500/80 mb-2 block">From</label>
                    <input type="text" name="from" required maxlength="100" placeholder="Nama kamu (boleh samaran)"
                ...
                <div>
                    <label
                        class="font-cinzel text-[0.6rem] tracking-widest uppercase text-gold-500/80 mb-2 block">To</label>
                    <input type="text" name="to" required maxlength="100" placeholder="Ditujukan ke siapa?" class="w-full bg-white/5 border border-gold-500/20 rounded-sm px-4 py-2.5
                                  text-sm text-zinc-200 placeholder-zinc-600
                                  focus:outline-none focus:border-gold-500/50 transition">
                </div>
                <div>
                    <label
                        class="font-cinzel text-[0.6rem] tracking-widest uppercase text-gold-500/80 mb-2 block">Pesan</label>
                    <textarea name="message" rows="4" required maxlength="500" placeholder="Tulis pesanmu di sini..."
                        class="w-full bg-white/5 border border-gold-500/20 rounded-sm px-4 py-2.5
                                     text-sm text-zinc-200 placeholder-zinc-600
    ...
    <footer class="border-t border-gold-500/15 py-10 px-6 lg:px-10">
        <div class="max-w-screen-xl mx-auto flex flex-col items-center gap-6">
            <!-- Logo Organisasi Ber-link ke Website HMIF -->
            <div class="flex items-center gap-6 justify-center">
                <a href="https://hmif.unikom.ac.id/" target="_blank" class="transition hover:scale-105">
            ...
            <!-- Sosial Media dengan Icon / Thumbnail dari Filament Storage -->
            <div class="flex flex-wrap justify-center items-center gap-6">
                @foreach($sosmed as $sm)
                    <a href="{{ $sm->url }}" target="_blank" class="flex items-center gap-2 font-cinzel text-[0.65rem] tracking-[0.25em] uppercase
                                              text-zinc-400 hover:text-gold-400 transition group">
                        @if(!empty($sm->thumbnail))
                            <img src="{{ asset('storage/' . $sm->thumbnail) }}" alt="{{ $sm->nama }}"
                                class="w-5 h-5 rounded-full object-cover border border-gold-500/30 group-hover:border-gold-400 transition">
                        @endif
                        <span>{{ $sm->nama }}</span>
                    </a>
                @endforeach
