...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-[1fr_auto_1.6fr] gap-4 mb-4 items-stretch">
        ...
        <div class="card-glass mobile-card rounded-lg p-6 text-center reveal flex flex-col justify-between">
            <div>
                ...
                @if($peserta->kelompok)
                    <p class="text-xs text-zinc-400 mt-1">Kelompok: <span
                            class="text-gold-400 font-semibold">{{ $peserta->kelompok->nama_kelompok }}</span></p>
                @else
                    <div class="mt-2">
                        @if($peserta->status)
        ...
        <div class="card-glass mobile-card rounded-lg p-6 flex flex-col justify-center items-center gap-3 reveal">
            ...
            @if($peserta->kelompok)
                ...
                @if($peserta->kelompok->url_grub)
                    <a href="{{ $peserta->kelompok->url_grub }}" target="_blank" class="theme-action theme-action-dark">
                        Gabung Grup
                    </a>
                @endif
                <a href="{{ asset('documents/Panduan_Peserta.pdf') }}" target="_blank" class="theme-action theme-action-gold">
                    Buku Panduan
                </a>
                <a href="{{ route('peserta.absensi') }}" class="theme-action theme-action-soft">
                    Absen
                </a>

            @elseif($peserta->status)
                {{-- Sudah ACC tapi belum pilih house: tombol buka modal --}}
                <p class="font-cinzel text-[0.6rem] tracking-widest uppercase text-gold-500 mb-1">Group Assignment</p>
                ...
                <button onclick="openHouseModal()" class="theme-action theme-action-gold cursor-pointer">
                    Pilih Grup
                </button>
            ...
            @else
                ...
                <div class="text-center px-2">
                    ...
                    <p class="text-xs text-zinc-500 leading-relaxed">Menunggu konfirmasi admin.<br>Tombol aksi akan muncul
                        setelah akun disetujui.</p>
                </div>
        ...
        <div class="schedule-card mobile-card rounded-lg p-6 reveal">
            ...
            <div class="space-y-3">
                ...
                @empty
                    <p class="text-xs text-black/60 font-cinzel tracking-widest uppercase text-center py-2">Belum ada agenda.
                    </p>
                @endforelse
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        {{-- Pengumuman --}}
        <div class="card-glass rounded-lg p-6 reveal space-y-4">
            <div class="flex items-center justify-between border-b border-gold-500/15 pb-3">
                <div class="flex items-center gap-2">
                    <span class="w-1.5 h-4 rounded-full shrink-0"
                        style="background: #C9A84C; box-shadow: 0 0 8px rgba(201,168,76,0.6);"></span>
                    <h3 class="font-cinzel font-bold text-white text-base tracking-wide">📣 Pengumuman Terbaru</h3>
                </div>
                @if($pengumumans->count() > 0)
                    <span class="text-[10px] font-mono text-zinc-500 uppercase tracking-widest">
                        {{ $pengumumans->count() }} Pengumuman
                    </span>
                @endif
            </div>
            ...
            @forelse($pengumumans as $index => $pengumuman)
                <div
                    class="p-4 rounded-md border border-gold-500/10 bg-white/[0.02] space-y-2 transition hover:border-gold-500/30">
                    <!-- Header Item Pengumuman -->
                    <div class="flex flex-wrap items-start justify-between gap-2">
                        <h4 class="font-bold text-gold-300 text-sm leading-snug">
                            {{ $pengumuman->judul }}
                        </h4>
                        <span class="text-[10px] font-mono text-zinc-500 shrink-0">
                            {{ $pengumuman->created_at ? $pengumuman->created_at->format('d M Y, H:i') : '' }} WIB
                        </span>
                    </div>

                    <!-- Isi Pengumuman (Mendukung Format Rich Text Filament) -->
                    <div
                        class="prose prose-invert max-w-none text-xs text-zinc-300 leading-relaxed prose-p:my-1 prose-a:text-gold-400">
                        {!! $pengumuman->konten !!}
                    </div>
                </div>
            @empty
                <!-- State Jika Belum Ada Pengumuman di Database -->
                <div class="flex items-center gap-2 py-2">
                    <span class="w-1 h-4 rounded-full shrink-0" style="background: #C9A84C;"></span>
                    <p class="text-sm text-zinc-500 italic">Belum ada pengumuman resmi saat ini.</p>
                </div>
            @endforelse
        </div>
...
@push('scripts')
    <style>
        ...
        @keyframes modalCelebrationGlow {
            0% {
                box-shadow: 0 0 0 rgba(201, 168, 76, 0), 0 32px 80px rgba(0, 0, 0, 0.8);
            }

            35% {
                box-shadow: 0 0 42px rgba(201, 168, 76, 0.38), 0 32px 80px rgba(0, 0, 0, 0.8);
            }

            100% {
                box-shadow: 0 0 18px rgba(201, 168, 76, 0.14), 0 32px 80px rgba(0, 0, 0, 0.8);
            }
        }
        ...
        .hm-scan-line .hm-scan-key {
            color: inherit;
        }

        .hm-scan-line .hm-scan-val {
        ...
        .hm-scan-line.active .hm-scan-val {
        ...
        @keyframes goldPulse {
            ...
            0%,
            100% {
                text-shadow: 0 0 14px rgba(201, 168, 76, 0.35), 0 2px 8px rgba(0, 0, 0, 0.6);
            }

            50% {
                text-shadow: 0 0 30px rgba(201, 168, 76, 0.75), 0 0 60px rgba(201, 168, 76, 0.22), 0 2px 8px rgba(0, 0, 0, 0.6);
            }
        }

        .hm-house-name {
            animation: houseReveal 0.9s cubic-bezier(0.16, 1, 0.3, 1) both, goldPulse 2.8s 0.9s ease-in-out infinite;
        }
        ...
        <div id="hm-modal">
            ...
            <div id="hm-sigil">
                <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <polygon points="100,18 116,68 168,68 126,98 142,148 100,118 58,148 74,98 32,68 84,68" fill="none"
                        stroke="rgba(201,168,76,0.15)" stroke-width="1" />
            ...
            <div id="hm-p1" class="hm-phase">
                ...
                <div class="hm-divider"></div>
                <p class="hm-sub mb-6">Grup-mu telah menanti.</p>
                <button class="hm-btn" onclick="hmBegin()">UNGKAP GRUP-MU</button>
            </div>
            ...
            <div id="hm-ps" class="hm-phase" style="display:none;">
                ...
                <div class="hm-divider"></div>
                <p class="hm-sub">Tarik napas dulu. Grup-mu sebentar lagi terungkap.</p>
            </div>
            ...
            <div id="hm-p3" class="hm-phase" style="display:none;">
                <p class="hm-label" style="color: rgba(150,220,150,0.7);">PENENTUAN GRUP SELESAI</p>
                <div class="hm-divider"></div>
                ...
                <div class="hm-divider"></div>
                <p class="hm-sub hm-journey mt-2 mb-5" style="color:rgba(201,168,76,0.5); font-size:0.6rem;">
                    PERJALANANMU DIMULAI DI SINI
                </p>
                <button class="hm-btn" onclick="hmEnter()">MASUK KE GRUP</button>
            </div>
